function h = viewannots(dataset, layerName,startAnnot,channels)
    ...
    if strcmp(choice,'Save anyway')
        ...
        for i=1:numel(labels)
            ...
            for p=1:length(annot)
                objs=[objs IEEGAnnotation.createAnnotations(annot(p).start,annot(p).stop,'Event','spike',annot(p).channels)];
            end
            if ~isempty(objs)
